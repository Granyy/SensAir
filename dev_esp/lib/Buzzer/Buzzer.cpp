...
void Buzzer::set_frequency(uint16_t frequency) {
	...
	buzzer_timer.duty_resolution = LEDC_TIMER_10_BIT, buzzer_timer.freq_hz =
			frequency, buzzer_timer.speed_mode = BUZZER_MODE;
...
void Buzzer::set_duty(uint16_t duty) {
	if (duty > 1024)
		ledc_set_duty(channel.speed_mode, channel.channel, 1024);
	else
